...
autoMergeGapMsInput?.addEventListener('input', () => {
  updateEditorSettings({ autoMergeGapMs: MaweSettings.clampAutoMergeGapMs(autoMergeGapMsInput.value) });
});
...
autoMergeShortCountInput?.addEventListener('input', () => {
  updateEditorSettings({ autoMergeShortCount: MaweSettings.clampAutoMergeShortCount(autoMergeShortCountInput.value) });
});
...
clickBehaviorSelect?.addEventListener('change', () => {
  updateEditorSettings({ clickBehavior: MaweSettings.normalizeClickBehavior(clickBehaviorSelect.value) });
  refreshClickBehaviorHint();
});
clickTargetSelect?.addEventListener('change', () => {
  updateEditorSettings({ clickTarget: MaweSettings.normalizeClickTarget(clickTargetSelect.value) });
});
...
function refreshMediaSeekInputStep(value = MaweSettings.EDITOR_SETTINGS.mediaSeekStepMs) {
  if (mediaSeekStepInput) mediaSeekStepInput.step = String(MaweSettings.mediaSeekStepForValue(value));
}
...
cueMoveStepInput?.addEventListener('change', () => {
  const value = MaweSettings.clampCueMoveStepMs(cueMoveStepInput.value);
...
ninjaSlashLengthInput?.addEventListener('change', () => {
  updateEditorSettings({ ninjaSlashLengthPercent: MaweSettings.clampNinjaSlashLength(ninjaSlashLengthInput.value) });
...
ninjaSlashRotateInput?.addEventListener('change', () => {
  updateEditorSettings({ ninjaSlashRotateAmplitude: MaweSettings.clampNinjaSlashRotateAmplitude(ninjaSlashRotateInput.value) });
...
function togglePlayback() {
  ...
  if (player.paused) {
    if (MaweJklPlayback.isDirectionMode()) player.playbackRate = Math.max(0.0625, Math.abs(MaweJklPlayback.getRate()));
...
function refreshMediaSeekStepHelp() {
  if (helpMediaSeekStep) helpMediaSeekStep.textContent = mediaSeekStepLabel(MaweSettings.EDITOR_SETTINGS.mediaSeekStepMs);
}
...
function syncMediaControls() {
  ...
  const active = hasMedia && (MaweJklPlayback.isReversePlaying() || !player.paused);
...
mediaStepBack?.addEventListener('click', () => seekMediaBy(-MaweSettings.EDITOR_SETTINGS.mediaSeekStepMs / 1000));
mediaStepForward?.addEventListener('click', () => seekMediaBy(MaweSettings.EDITOR_SETTINGS.mediaSeekStepMs / 1000));
...
document.addEventListener('keydown', (e) => {
  ...
  if (MaweJklPlayback.isDirectionMode()) {
    if (k === 'k') {
      const wasPlaying = MaweJklPlayback.isReversePlaying() || !player.paused;
      if (!wasPlaying) {
...
function scheduleAutoSave() {
  ...
  autoSaveIntervalInput.addEventListener('change', () => {
    updateEditorSettings({ autoSaveIntervalSeconds: MaweSettings.clampAutoSaveInterval(autoSaveIntervalInput.value) });
    sync();
    scheduleAutoSave();
  });
  scheduleAutoSave();
