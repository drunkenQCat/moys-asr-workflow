...
const manifest = fs.readFileSync(new URL('../web/editor-scripts.txt', import.meta.url), 'utf8')
  .split('\n')
  .map((line) => line.trim())
  .filter((line) => line && !line.startsWith('#'));
const blockStart = manifest.indexOf('editor/waveform/namespace.js');
const blockEnd = manifest.indexOf('editor/waveform/compat-surface.js');
assert.ok(
